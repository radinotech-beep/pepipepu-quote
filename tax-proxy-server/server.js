...
업태와 업종은 여러 개가 적혀 있어도 맨 첫 번째(맨 위) 항목 하나만 반환하세요.
예: 업태가 "도소매, 제조, 서비스"라면 "도소매"만 반환하세요.
예: 업종이 "컴퓨터및주변기기, 전자제품"이라면 "컴퓨터및주변기기"만 반환하세요.
업태는 "업태" 또는 "業態" 라벨 옆의 값을 우선 확인하세요.
업종은 "종목" 또는 "업종" 라벨 옆의 값을 우선 확인하세요.
사업자등록증에서는 업태/업종이 표 형태로 배치되는 경우가 많으므로, 표에서 해당 라벨의 첫 번째 행/첫 번째 값을 기준으로 추출하세요.
